...
def test_trainer_loss_selection(loss_user, binary, criterion_):
    ...
    t = SegTrainer(X_train, y_train, X_test, y_test,
...
def test_im2spec_trainer_determinism(latent_dim):
    ...
    t1 = ImSpecTrainer(X_train, y_train, X_test, y_test,
                       latent_dim=latent_dim, training_cycles=5,
                       batch_size=4, seed=1)
    t1.run()
    loss1 = t1.loss_acc["train_loss"][-1]
    t2 = ImSpecTrainer(X_train, y_train, X_test, y_test,
                       latent_dim=latent_dim, training_cycles=5,
                       batch_size=4, seed=1)
    t2.run()
    loss2 = t2.loss_acc["train_loss"][-1]
...
def test_init_unet_bn(bn, n_bn, layers):
    ...
    t = SegTrainer(X_train, y_train, X_test, y_test,
                   model="dilUnet", use_batchnorm=bn,
                   layers=layers, batch_size=4)
...
def test_init_unet_dropouts(dropouts, n_dropouts):
    ...
    y_train, y_test = gen_image_labels(binary=False)
    t = SegTrainer(X_train, y_train, X_test, y_test,
                   model="dilUnet", use_dropouts=dropouts,
                   batch_size=4)
    n_dropouts_ = 0
...
def test_init_dilnet_dropouts(dropouts, n_dropouts):
    ...
    y_train, y_test = gen_image_labels(binary=False)
    t = SegTrainer(X_train, y_train, X_test, y_test,
                   model="dilnet", use_dropouts=dropouts,
                   batch_size=4)
    n_dropouts_ = 0
    for c in t.net.children():
        if 'Dropout' in str([m for m in c.named_modules()]):
...
def test_init_dilnet_layers(layers):
    ...
    y_train, y_test = gen_image_labels(binary=False)
    t = SegTrainer(X_train, y_train, X_test, y_test,
                   model="dilnet", layers=layers, batch_size=4)
    n_bn_ = len([k for k in t.net.state_dict().keys() if 'running_mean' in k])
    layers_ = len([k for k in t.net.state_dict().keys() if "weight" in k])
...
@pytest.mark.parametrize(
    ...
    [("dilUnet", 25, [25, 50, 100, 200, 100, 50, 25]),
     ("dilnet", 25, [25, 50, 50, 25])])
def test_init_segmodel_filters(model, nb_filters_in, nb_filters_layers):
    from atomai.nets.blocks import upsample_block
    X_train, X_test = gen_image_data()
    y_train, y_test = gen_image_labels(binary=False)
    t = SegTrainer(X_train, y_train, X_test, y_test,
                   model=model, nb_filters=nb_filters_in,
                   use_batchnorm=False, batch_size=4)
    filters_ = []
    for child in t.net.children():
        if isinstance(child, upsample_block):
            continue
        filt_ = []
...
def test_init_imspec_bn(bn, encoder_layers, decoder_layers, n_bn):
    X_train, X_test = gen_image_data()
    y_train, y_test = gen_spectra()
    t = ImSpecTrainer(X_train, y_train, X_test, y_test,
                      nblayers_encoder=encoder_layers,
                      nblayers_decoder=decoder_layers,
                      use_batchnorm=bn, batch_size=4, training_cycles=1)
    n_bn_ = len([k for k in t.net.state_dict().keys() if 'running_mean' in k])
    assert_equal(n_bn_, n_bn)
...
def test_init_im2spec_edownsample(downsample_factor, output_dim):
    ...
    y_train, y_test = gen_spectra()
    t = ImSpecTrainer(X_train, y_train, X_test, y_test,
                      encoder_downsampling=downsample_factor,
                      batch_size=4, training_cycles=1)
...
def test_im2spec_filters(efilt, dfilt, filters_e_sorted, filters_d_sorted):
    from atomai.nets import signal_decoder, signal_encoder
    X_train, X_test = gen_image_data()
    y_train, y_test = gen_spectra()
    t = ImSpecTrainer(X_train, y_train, X_test, y_test,
                      nbfilters_encoder=efilt, nbfilters_decoder=dfilt,
                      batch_size=4, training_cycles=1)
    filters_e, filters_d = [], []
    for child in t.net.children():
        if isinstance(child, signal_encoder):
            filt_ = []
            for p in child.state_dict().values():
                if len(p.shape) == 4:
                    filt_.append(p.shape[0])
            filters_e.extend(filt_)
        elif isinstance(child, signal_decoder):
            filt_ = []
            for p in child.state_dict().values():
                if len(p.shape) == 3:
                    filt_.append(p.shape[0])
